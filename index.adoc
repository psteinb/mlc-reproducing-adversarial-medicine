...
[asciimath] 
++++ 
x^{t+1} = \prod_{x+S} (x^{t} + \epsilon \sign(\nabla_x L(\theta,x,y)))
++++
...
[quote]
____
... black box attacks were performed by crafting the attack against an independently-trained model with the same architecture and then transferring the resultant adversarial
examples to the victim.
____

IMPORTANT: <<Biggio18>> defines this as Grey-Box Attack.
